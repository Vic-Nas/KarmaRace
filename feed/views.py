...
def _webhook_stats(task):
    ...
    failure_rate_percent = int(round(tried_failed / denominator * 100))
...
def feed(request):
    ...
    task = _get_pinned_task(request, archive=archive, task_types=selected_task_types, obligations=obligations)
    if task is None:
        task = _get_feed_task(
            request.user, completion=completion, archive=archive,
            task_types=selected_task_types, obligations=obligations,
        )
...
def check(request, task_id):
    ...
    if completion.state == TaskCompletion.State.CONFIRMED:
        ...
        return respond(
            TaskCompletion.State.CONFIRMED, msg('ALREADY_CONFIRMED'),
    ...
    try:
        ...
        try:
            ...
            if completion.state == TaskCompletion.State.CONFIRMED:
                from karma.services import get_balance
                return respond(
                    TaskCompletion.State.CONFIRMED, msg('QUEUE_UNAVAILABLE_CONFIRMED'),
