...
def _exchange_google_code_for_email(code, redirect_uri, code_verifier=None):
	"""Exchange Google OAuth code for user's email address."""
	flow = _build_flow(redirect_uri)

	# Re-attach the code_verifier so Google can validate the PKCE handshake.
	# The verifier was generated during verify_email_start and stored in the
	# session; without it the token exchange fails with invalid_grant.
	if code_verifier:
...
def verify_email_start(request):
	...
	# Save the code_verifier generated by authorization_url() so the callback
	# can send it back to Google during token exchange (PKCE requirement).
	request.session['email_verify_code_verifier'] = flow.code_verifier
...
def _try_exchange_email(request, code):
	try:
		redirect_uri = request.build_absolute_uri(reverse('verify_email_callback'))
		# Pop the verifier from the session (single-use, same as state).
		code_verifier = request.session.pop('email_verify_code_verifier', None)
		return _exchange_google_code_for_email(code, redirect_uri, code_verifier)
