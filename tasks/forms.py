...
from .services import get_user_github_repo_choices
...
class TaskForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.user is not None:
            self.github_repo_choices, self.github_repo_load_error = get_user_github_repo_choices(self.user)
    ...
    def clean_target_id(self):
        ...
        if task_type in (Task.Type.GITHUB_STAR, Task.Type.GITHUB_FORK):
            if self.github_repo_load_error:
                raise forms.ValidationError(self.github_repo_load_error)
